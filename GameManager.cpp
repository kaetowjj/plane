...
GameManager::GameManager(void)
{
	...
	m_MenuStrings[1].assign("音乐欣赏");
	m_MenuStrings[2].assign("排行榜");
	m_MenuStrings[3].assign("退出游戏");
...
void GameManager::ShowMenu()
{
	...
	for (int i=0; i<m_iSelectionMax; i++)
	{
		if (i == m_iFocus)
		{
			...
			TextOut(g_hdcBuffer, 155, textY, m_MenuStrings[i].c_str(), (int)m_MenuStrings[i].length());
			textY += 30;
			SetTextColor(g_hdcBuffer, oldColor);
		}
		else
